...
router.post('/:id/run', requireRole('OWNER', 'ADMIN', 'MANAGER'), asyncHandler(async (request, response) => {
  ...
  const [discoveryCredential, aiCredential] = await Promise.all([
    prisma.providerCredential.findFirst({ where: { organizationId, provider: { in: ['GOOGLE_PLACES', 'GOOGLE_CUSTOM_SEARCH', 'SERPER', 'BRAVE_SEARCH'] }, isActive: true }, select: { id: true } }),
  ...
  if (!discoveryCredential) throw new AppError(409, 'Connect Google Places, Google Search, Serper, or Brave Search in Settings before running a search', 'DISCOVERY_PROVIDER_REQUIRED');
  ...
  await prisma.campaign.update({ where: { id: campaign.id }, data: { status: 'QUEUED', failureReason: null } });
