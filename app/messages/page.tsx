...
export default function MessagesPage() {
  ...
  const suggestion = selectedLead ? `Thanks for the conversation. I can show ${selectedLead.name} a focused demo built around your current workflow. Would a short call this week be useful?` : '';
  ...
  async function sendDraft(event: FormEvent) {
    ...
    const email = selectedLead.contacts.find(contact => contact.email?.trim())?.email?.trim();
    const whatsapp = selectedLead.contacts.find(contact => contact.whatsapp?.trim())?.whatsapp?.trim();
    const recipient = editing?.recipient || email || whatsapp;
    if (!recipient) { setError('This lead has no email or WhatsApp destination'); return; }
    sending.current = true;
    setError('');
    setNotice('Submitting message for delivery…');
    try {
      let message = editing;
      if (message) await apiRequest(`/outreach/${message.id}`, { method: 'PATCH', body: JSON.stringify({ body: text }) });
      else {
        const created = await apiRequest<{ message: Message }>('/outreach', { method: 'POST', body: JSON.stringify({ leadId: selectedLead.id, channel: email ? 'EMAIL' : 'WHATSAPP', recipient, subject: `Following up with ${selectedLead.name}`, body: text }) });
        message = created.message;
        // Retain the draft if submission fails so another click retries the same message.
        setEditing(message);
      }
      await apiRequest(`/outreach/${message.id}/approve`, { method: 'POST', body: JSON.stringify({}) });
      setText(''); setEditing(null);
      await load();
      setNotice('Message queued for delivery. SENT means the provider accepted it.');
